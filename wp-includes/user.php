...
function wp_insert_user( $userdata ) {
	 ...
	 */
	$meta['company'] = empty( $userdata['company'] ) ? '' : $userdata['company'];
	$meta['country'] = empty( $userdata['country'] ) ? '' : $userdata['country'];
	$meta['state'] = empty( $userdata['state'] ) ? '' : $userdata['state'];
	$meta['city'] = empty( $userdata['city'] ) ? '' : $userdata['city'];
	$meta['accountType'] = empty($userdata['accountType'] ) ? '' : $userdata['accountType'];
	$meta['instagram'] = empty( $userdata['instagram'] ) ? '' : $userdata['instagram'];
	$meta['twitter'] = empty( $userdata['instagram'] ) ? '' : $userdata['instagram'];
    $meta['membershipType'] = empty($userdata['membershipType']) ? '' : $userdata['membershipType'];
    $meta['remainingMessages'] = empty($userdata['remainingMessages']) ? '' : $userdata['remainingMessages'];
    $meta['maximumMessages'] = empty($userdata['maximumMessages']) ? '' : $userdata['maximumMessages'];
	/*$meta['address'] = empty( $userdata['address'] ) ? '' : $userdata['address'];
	$meta['address2'] = empty( $userdata['address2'] ) ? '' : $userdata['address2'];
	$meta['zip'] = empty( $userdata['zip'] ) ? '' : $userdata['zip'];*/
	$meta['phoneNo'] = empty( $userdata['phoneNo'] ) ? '' : $userdata['phoneNo'];
	...
	$user_guid = wp_generate_password( 25, false );
	...
	$compacted = compact( 'user_pass', 'user_email', 'user_url', 'user_nicename', 'display_name', 'user_registered', 'user_guid', 'username' );
...
function wp_create_influencer($email, $password, $firstname, $lastname, $company, $country, $state, $city, $phoneNo, $passwordIssue, $username, $accountType, $membershipType, $remainingMessages, $maximumMessages){
	$user_login = wp_slash( $email );
	$user_email = wp_slash( $email );
	if($passwordIssue == 1)
		return new WP_Error('password_issue', __('There seems to be a problem with your password. Please check your password before submitting.') );
	$user_pass = $password;
	$first_name = $firstname;
	$last_name = $lastname;
	
	$userdata = compact('user_login', 'user_email', 'user_pass', 'first_name', 'last_name', 'company', 'country', 'state', 'city', 'phoneNo', 'username', 'accountType', 'membershipType', 'remainingMessages', 'maximumMessages');
	wp_insert_user($userdata);
	return new WP_Error('password_issue', __('Your account has been submitted for approval, you will recieve an email shortly.') );
}

function wp_update_athlete($userID, $email, $firstname, $lastname, $company, $country, $state, $city, $address, $address2, $zip, $phoneNo){
	$ID = $userID;
	$user_email = wp_slash( $email );
	$first_name = $firstname;
	$last_name = $lastname;
	
	$userdata = compact('ID', 'user_email', 'first_name', 'last_name', 'company', 'country', 'state', 'city', 'address', 'address2', 'zip', 'phoneNo');
